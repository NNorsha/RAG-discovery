...
### Cloud Studio 部署(腾讯云在线 IDE)

如果你希望保留在腾讯云 Cloud Studio 中开发、预览并分享链接,按下面步骤操作:

1. 登录 [Cloud Studio](https://cloudstudio.net/home),新建工作空间。
2. 导入本仓库(或直接把本地 `RAG-discovery-main` 文件夹上传到工作空间)。
3. 在工作空间设置「环境变量」中添加:
   - `LLM_BASE_URL=https://open.bigmodel.cn/api/paas/v4/`
   - `LLM_MODEL=glm-4-flash`
...
4. 打开终端,运行:
   ```bash
   bash cloudstudio-start.sh
   ```
5. 等待 Streamlit 启动后,Cloud Studio 会在右上角提示「端口 8080」的预览/访问链接,点击即可复制网址。
   也可在左侧「端口」面板中查看并「公开访问」该链接。

> 提示:Cloud Studio 的预览链接在 workspace 运行期间有效;如需一个长期稳定的公网域名,请把项目部署到 Render、Streamlit Cloud 或腾讯云 CVM/SCF。
